...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        for batch_i, batch in enumerate(bar):
            ...
            with dt[3]:
                ...
                preds = self.check_warning(preds,height,width)
    ...
    def check_warning(self, preds, height, width):
        result = []
        now_pred = []
        far = 0.18
        middle = 0.145
        ...
        for pred in preds:
            for xyxy in pred:
                ...
                x_max = xyxy[2]
                y_max = xyxy[3]
                mask_thres1 = np.zeros((height,width),np.uint8)
                cv2.ellipse(mask_thres1,(int(width/2),height),(int(height * far * ellipse_value),int(height * far)),0,angle_center - angle_near,angle_center + angle_near,bitmask,-1)
                np_size = mask_thres1[int(y_min):int(y_max),int(x_min):int(x_max)].shape
                if np.any((mask_thres1[int(y_min):int(y_max),int(x_min):int(x_max)] & np.ones(np_size, np.uint8)) > 0):
                    now_pred.append(xyxy.cpu().numpy())
            now_pred = np.array(now_pred)
            now_pred = torch.Tensor(now_pred)
            now_pred = now_pred.to('cuda')
            result.append(now_pred)
            now_pred = []
        return result
